...
export function TransactionForm({ type, open: externalOpen, setOpen: setExternalOpen, transaction }: TransactionFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: `Failed to ${isEditMode ? 'update' : 'add'} transaction.` });
    } finally {
      setIsLoading(false);
    }
  }
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <FormField control={form.control} name="description" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Input placeholder={`e.g. ${type === 'income' ? 'Monthly Salary' : 'Coffee'}`} {...field} /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
          ...
          <DialogFooter>
            <Button type="submit" disabled={isLoading}>
              {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {isEditMode ? "Save Changes" : "Save"}
            </Button>
          </DialogFooter>
        </form>
      </Form>
  ...
  if (isEditMode) return <Dialog open={open} onOpenChange={setOpen}>{dialogContent}</Dialog>;

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" variant={type === "income" ? "outline" : "default"}>
          <PlusCircle className="mr-2 h-4 w-4" /> {type === "income" ? "Add Income" : "Log Expense"}
        </Button>
      </DialogTrigger>
      {dialogContent}
    </Dialog>
